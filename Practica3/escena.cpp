...
void Escena::draw() {
    ...
    if (normales == 1){
        if (taulaBillar!=NULL){
            taulaBillar->activarNormales();
        }
        if (plabase != NULL){
            plabase -> activarNormales();
        }
        if (bola != NULL){
            bola -> activarNormales();
        }
        if (conjuntboles != NULL){
            conjuntboles->normales = 1;
        }
    }
    else{
        if (taulaBillar!=NULL){
            taulaBillar->activarNormales2();
        }
        if (plabase != NULL){
            plabase -> activarNormales2();
        }
        if (bola != NULL){
            bola -> activarNormales2();
        }
        if (conjuntboles != NULL){
            conjuntboles->normales = 2;
        }
    }

    if (texturas == true){
        if (taulaBillar!=NULL){
            taulaBillar->activarTextura();
            taulaBillar->draw();
        }
        if (plabase != NULL){
            plabase -> activarTextura();
            plabase -> draw();
        }
        if (bola != NULL){
            bola -> activarTextura();
        ...
        if (conjuntboles != NULL){
            conjuntboles->texturas = true;
            conjuntboles -> draw();
        }
    }
    else{
        if (taulaBillar!=NULL){
            taulaBillar->desactivarTextura();
            taulaBillar->draw();
        }
        if (plabase != NULL){
            plabase -> desactivarTextura();
            plabase -> draw();
        }
        if (bola != NULL){
            bola -> desactivarTextura();
            bola -> draw();
        }
        if (conjuntboles != NULL){
            conjuntboles->texturas = false;
            conjuntboles -> draw();
        }
    }
...
void Escena::crearLuces(){
    ...
    luces = new ConjuntLlums();

    luces->ambienteGlobal = 0.01;

    Llum* direccional = new Llum("direccional", 0);
    luces->add(direccional);

    Llum* puntual = new Llum("puntual", 1);
    luces->add(puntual);

    Llum* spotlight = new Llum("spotlight", 2);
    luces->add(spotlight);

}

void Escena::activarTextura(){

    texturas = true;

}

void Escena::desactivarTextura(){

    texturas = false;

}

void Escena::activarNormales(){

    normales = 1;

}
void Escena::activarNormales2(){
