...
void Objecte::toGPU(QGLShaderProgram *pr){
    ...
    normalLocation = program->attributeLocation("vNormal");

    program->enableAttributeArray(vertexLocation);
    program->setAttributeBuffer("vPosition", GL_FLOAT, 0, 4);

    program->enableAttributeArray(coordTextureLocation);
    program->setAttributeBuffer("vCoordTexture", GL_FLOAT, sizeof(point4) * Index, 2);

    program->enableAttributeArray(normalLocation);
    program->setAttributeBuffer("vNormal", GL_FLOAT, sizeof(point4) * Index + sizeof(vec2) * Index, 4);
...
void Objecte::draw(){
    ...
    glBufferData( GL_ARRAY_BUFFER, sizeof(point4) * Index + sizeof(vec2) * Index + sizeof(vec4) * Index, NULL, GL_STATIC_DRAW );

    // per si han canviat les coordenades dels punts
    glBufferSubData( GL_ARRAY_BUFFER, 0, sizeof(point4) * Index, points );
    glBufferSubData( GL_ARRAY_BUFFER, sizeof(point4) * Index, sizeof(vec2) * Index, vertexsTextura );
    if (normales == 1){
        glBufferSubData( GL_ARRAY_BUFFER, sizeof(point4) * Index + sizeof(vec2) * Index, sizeof(vec4) * Index, normals );
    }else{
        glBufferSubData( GL_ARRAY_BUFFER, sizeof(point4) * Index + sizeof(vec2) * Index, sizeof(vec4) * Index, normals2 );
    }
    program->setAttributeBuffer("vCoordTexture", GL_FLOAT, sizeof(point4) * Index, 2);
    program->setAttributeBuffer("vNormal", GL_FLOAT, sizeof(point4) * Index + sizeof(vec2) * Index, 4);
...
void Objecte::activarTextura(){
    texture->bind(0);
}
void Objecte::desactivarTextura(){
    texture->release();
}
void Objecte::activarNormales(){
    normales = 1;
}
void Objecte::activarNormales2(){
    normales = 2;
}
