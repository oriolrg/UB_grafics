...
void
GLWidget::InitShader(const char* vShaderFile, const char* fShaderFile, QGLShaderProgram* program)
{           
...
// Carrega dels shaders i posa a punt per utilitzar els programes carregats a la GPU
    ...
    // Shader sin iluminacion
    QGLShaderProgram* program0 = new QGLShaderProgram(this);
    InitShader( "://vshader0.glsl", "://fshader0.glsl", program0 );
    programs.push_back(program0);

    // Shader Flat/Gouraud
    QGLShaderProgram* program1 = new QGLShaderProgram(this);
    InitShader( "://vshader1.glsl", "://fshader1.glsl", program1 );
    programs.push_back(program1);

    // Shader Phong Shading
    QGLShaderProgram* program2 = new QGLShaderProgram(this);
    InitShader( "://vshader2.glsl", "://fshader2.glsl", program2 );
    programs.push_back(program2);

    // Shader Toon Shading
    QGLShaderProgram* program3 = new QGLShaderProgram(this);
    InitShader( "://vshader3.glsl", "://fshader3.glsl", program3 );
...
void GLWidget::paintGL()
{
   ...
   esc->camGeneral->toGPU(programs[shader]);

   esc->luces->setAmbientToGPU(programs[shader]);
   esc->luces->toGPU(programs[shader]);
...
void GLWidget::newConjuntBoles()
{
    ...
    for (i = 0; i < 15; i++)
    {
        ...
        obj->boles[i]->toGPU(programs[shader]);
    }
    esc->addObjecte(obj);
    updateGL();
