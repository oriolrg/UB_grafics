...
{
    // CAL IMPLEMENTAR
    // CODI A MODIFICAR DURANT LA PRACTICA 2 OK
    ...
    vs.vrp.x = capsaMinima.pmin.x + (capsaMinima.a / 2.);
    vs.vrp.y = capsaMinima.pmin.y + (capsaMinima.h / 2.);
    vs.vrp.z = capsaMinima.pmin.z + (capsaMinima.p / 2.);
...
void Camera::toGPU(QGLShaderProgram *program)
{
    ...
    this->setModelViewToGPU(program, modView);
    this->setProjectionToGPU(program, proj);
}
...
void Camera::CalculaMatriuProjection()
{
    ...
    GLfloat left = wd.pmin[0];
    GLfloat right = wd.pmin[0]+wd.a;
    GLfloat bottom = wd.pmin[1];
    GLfloat top = wd.pmin[1]+wd.h;
    GLfloat zNear = piram.dant;
    GLfloat zFar = piram.dpost;
